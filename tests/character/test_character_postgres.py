...
from aiavatar.character.repository.postgres import (
    PostgreSQLActivityRepository,
    PostgreSQLCharacterRepository
)
...
DB_CONN_STR = f"postgresql://{AIAVATAR_DB_USER}:{AIAVATAR_DB_PASSWORD}@localhost:{AIAVATAR_DB_PORT}/aiavatar"
...
@pytest.mark.asyncio
async def test_repository_types_are_postgresql(ctx):
    """Verify that PostgreSQL repositories are being used, not SQLite."""
    assert isinstance(ctx.service.character, PostgreSQLCharacterRepository), \
        f"Expected PostgreSQLCharacterRepository, got {type(ctx.service.character).__name__}"
    assert isinstance(ctx.service.activity, PostgreSQLActivityRepository), \
        f"Expected PostgreSQLActivityRepository, got {type(ctx.service.activity).__name__}"
