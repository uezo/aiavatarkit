...
async def detector(test_output_dir):
    """
    Create AmazonTranscribeStreamSpeechDetector with real AWS connection
    """
    detector = AmazonTranscribeStreamSpeechDetector(
        **AWS_CREDENTIALS,
        aws_language="ja-JP",
    ...
    for session_id in list(detector.recording_sessions.keys()):
        await detector.delete_session(session_id)
...
async def send_audio_and_wait_for_recognition(detector, session_id: str, wave_data: bytes, chunk_size: int = 3200, timeout: float = 5.0):
    """
    Send audio data and then send silence while waiting for Amazon Transcribe recognition.
    This helper ensures we keep calling process_samples to detect the transcription result.
    """
    # Send speech audio
...
async def test_process_samples_speech_detection_with_text(detector, stt_wav_path):
    """
    Test to verify that when speech is detected, Amazon Transcribe recognizes the text
    and on_speech_detected is called with the recognized text.
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
...
async def test_process_stream_with_text(detector, stt_wav_path):
    """
    Test stream processing via process_stream with real Amazon Transcribe recognition.
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
...
async def test_on_recording_started_callback(stt_wav_path):
    """
    Test that on_recording_started callback is triggered when Amazon Transcribe starts recognizing.
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
    callback_calls = []
...
async def test_session_reset_and_delete(detector, stt_wav_path):
    ...
    # Verify session was created with Transcribe streaming started
    session = detector.get_session(session_id)
    assert session.transcribe_stream is not None
    assert session.event_handler_task is not None
    ...
    assert len(session.preroll_buffer) > 0
    # Transcribe streaming should still be active
    assert session.transcribe_stream is not None
    assert session.event_handler_task is not None
...
@pytest.mark.asyncio
async def test_session_data(detector):
    ...
    # Cleanup
    await detector.delete_session(session_id_1)
...
async def test_on_speech_detecting_callback(detector, stt_wav_path):
    ...
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
    from aiavatar.sts.vad.amazon_transcribe_stream import RecordingSession
    ...
    # on_speech_detecting should have been called at least once (during partial results)
...
def test_detector_inherits_from_speech_detector():
    """
    Test that AmazonTranscribeStreamSpeechDetector properly inherits from SpeechDetector.
    """
    ...
    # Check Amazon Transcribe-specific attributes
    assert hasattr(detector, 'aws_region')
    assert hasattr(detector, 'aws_language')
...
def test_recording_session_attributes():
    ...
    # Check Amazon Transcribe-specific attributes
    assert hasattr(session, 'transcribe_stream')
    assert hasattr(session, 'event_handler_task')
    ...
    assert session.transcribe_stream is None
    assert session.event_handler_task is None
...
def test_get_config(detector):
    ...
    assert config["aws_language"] == "ja-JP"
...
# ============================================================
# Tests for silence_duration_threshold / accumulated_texts / max_duration
# ============================================================


def test_recording_session_accumulated_texts_init():
    """
    Test that RecordingSession initializes accumulated_texts and silence_timer_task.
    """
    from aiavatar.sts.vad.amazon_transcribe_stream import RecordingSession

    session = RecordingSession("test", preroll_buffer_size=10)

    assert session.accumulated_texts == []
    assert session.silence_timer_task is None


def test_recording_session_reset_clears_accumulated_state():
    """
    Test that reset() clears accumulated_texts and cancels silence_timer_task.
    """
    from aiavatar.sts.vad.amazon_transcribe_stream import RecordingSession

    session = RecordingSession("test", preroll_buffer_size=10)
    session.accumulated_texts = ["hello", "world"]
    session.is_recording = True
    session.buffer.extend(b'\x00' * 100)
    session.record_duration = 3.0
...
@pytest.mark.asyncio
async def test_recording_session_cancel_silence_timer():
    """
    Test that cancel_silence_timer properly cancels a running timer.
    """
    from aiavatar.sts.vad.amazon_transcribe_stream import RecordingSession
    ...
    session.silence_timer_task = asyncio.create_task(dummy_timer())
    assert not session.silence_timer_task.done()
...
@pytest.mark.asyncio
async def test_recording_session_cancel_silence_timer_already_done():
    """
    Test that cancel_silence_timer is safe when the timer has already completed.
    """
    from aiavatar.sts.vad.amazon_transcribe_stream import RecordingSession

    session = RecordingSession("test", preroll_buffer_size=10)

    # Create a timer that finishes immediately
    async def instant():
        pass

    session.silence_timer_task = asyncio.create_task(instant())
    await asyncio.sleep(0.01)  # Let it finish
    assert session.silence_timer_task.done()

    # cancel_silence_timer should not raise
    session.cancel_silence_timer()


@pytest.mark.asyncio
async def test_silence_duration_threshold_accumulates_text(stt_wav_path):
    """
    Test that with a high silence_duration_threshold, text is accumulated
    across multiple final results and combined into a single speech_detected call.
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
    detector = AmazonTranscribeStreamSpeechDetector(
        **AWS_CREDENTIALS,
        aws_language="ja-JP",
        silence_duration_threshold=10.0,  # Very high - won't fire by silence alone
        max_duration=5.0,  # Will fire by max_duration after accumulation
        sample_rate=16000,
        channels=1,
        debug=True
    )
    ...
    # Send silence until max_duration triggers or we get a result
    silence_chunk = b'\x00' * chunk_size
    for _ in range(100):
        await detector.process_samples(silence_chunk, session_id)
        await asyncio.sleep(0.05)
    ...
    assert len(detected_results) >= 1, "No speech detected"
...
@pytest.mark.asyncio
async def test_on_speech_detecting_includes_accumulated_text(stt_wav_path):
    """
    Test that on_speech_detecting callback receives accumulated final texts
    prepended to the current partial text.
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
    from aiavatar.sts.vad.amazon_transcribe_stream import RecordingSession
    ...
    detector = AmazonTranscribeStreamSpeechDetector(
        **AWS_CREDENTIALS,
        aws_language="ja-JP",
        silence_duration_threshold=10.0,  # High threshold so timer doesn't fire
        max_duration=5.0,
        sample_rate=16000,
        channels=1,
        debug=True
    ...
    @detector.on_speech_detecting
    async def on_detecting(text: str, session: RecordingSession):
        detecting_calls.append(text)

    @detector.on_speech_detected
    async def on_speech_detected(recorded_data: bytes, text: str, metadata: dict, recorded_duration: float, session_id: str):
        detected_results.append({"text": text})

    session_id = "test_detecting_accumulated"
    ...
    # on_speech_detecting should have been called with partial texts
    assert len(detecting_calls) >= 1, "on_speech_detecting was not triggered"

    # If there were accumulated texts when a partial came in,
    # the detecting text should contain the accumulated prefix.
    # At minimum, the last detecting call should be a partial of the recognized text.
    final_text = detected_results[0]["text"] if detected_results else ""
    # At least one detecting call should be a prefix/substring of the final text
    assert any(
        d in final_text or final_text.startswith(d[:5])
        for d in detecting_calls
    ), f"No detecting call is a prefix of final text '{final_text}'. Calls: {detecting_calls}"
...
@pytest.mark.asyncio
async def test_max_duration_triggers_with_accumulated_text(stt_wav_path):
    """
    Test that max_duration triggers speech_detected with accumulated text
    when recording exceeds the max duration.
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
    detector = AmazonTranscribeStreamSpeechDetector(
        **AWS_CREDENTIALS,
        aws_language="ja-JP",
        silence_duration_threshold=30.0,  # Very high - silence timer won't fire
        max_duration=3.0,  # Low max - should trigger within test
        sample_rate=16000,
        channels=1,
        debug=True
    )
    ...
    # Keep sending silence to grow record_duration past max_duration
    silence_chunk = b'\x00' * chunk_size
    ...
    assert len(detected_results) >= 1, "max_duration did not trigger speech_detected"
    result = detected_results[0]
    assert "こんにちは" in result["text"], f"Expected 'こんにちは', got: {result['text']}"
    # Duration should be around max_duration (3.0s), with some tolerance
    assert result["duration"] >= 2.5, f"Duration too short: {result['duration']}"

    # Session should have been reset (not recording anymore)
    session = detector.get_session(session_id)
    assert session.is_recording is False
    assert len(session.accumulated_texts) == 0
...
@pytest.mark.asyncio
async def test_max_duration_no_accumulated_text_resets(detector):
    """
    Test that max_duration resets the session when there's no accumulated text.
    """
    session_id = "test_max_no_text"
    session = detector.get_session(session_id)

    # Simulate recording state with no accumulated texts
    session.is_recording = True
    session.buffer.extend(b'\x00' * 100)
    session.record_duration = detector.max_duration - 0.01

    # Start transcribe streaming (needed for process_samples)
    await detector._start_transcribe_streaming(session)

    # Send a chunk that pushes past max_duration
    chunk_duration = 0.1  # 100ms
    chunk_bytes = int(chunk_duration * detector.sample_rate * detector.channels * 2)
    chunk = b'\x00' * chunk_bytes

    await detector.process_samples(chunk, session_id)

    # Session should be reset (no accumulated text)
    assert session.is_recording is False
    assert len(session.buffer) == 0
    assert session.record_duration == 0
    assert len(session.accumulated_texts) == 0

    # No speech_detected should have been triggered
    assert len(detector._detected_results) == 0


@pytest.mark.asyncio
async def test_silence_threshold_zero_triggers_immediately(stt_wav_path):
    """
    Test that silence_duration_threshold=0 triggers speech_detected immediately
    on final result (original behavior).
    NOTE: This test actually calls Amazon Transcribe's Streaming API and consumes credits.
    """
    detector = AmazonTranscribeStreamSpeechDetector(
        **AWS_CREDENTIALS,
        aws_language="ja-JP",
        silence_duration_threshold=0,
        max_duration=20.0,
        sample_rate=16000,
        channels=1,
        debug=True
    ...
    session_id = "test_zero_threshold"
